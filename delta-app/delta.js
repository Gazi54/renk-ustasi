...
$("d-ocr-form").onclick = async () => {
  ...
  bul.forEach(m => { if (!gor.has(m[1])) { gor.add(m[1]); sat.push(`${m[1]} = ${m[2].replace(",", ".")}`); } });
  $("d-form").value = sat.join("\n");
  $("d-sonuc").textContent = "Fotoğraftan okundu — KONTROL ET, yanlış varsa düzelt:\n" + sat.join("\n");
};
$("d-ocr-lab").onclick = async () => {
  ...
  metin.split("\n").map(s => s.trim()).filter(Boolean).forEach(s => {
    ...
    s.split(/\s+/).slice(1).forEach(w => {
      let neg = false;
      if (/^[*\-–]/.test(w)) { neg = true; w = w.replace(/^[*\-–]/, ""); }
      const m2 = w.match(/(\d+)[.,](\d+)/);
      if (m2) { const v = parseFloat(m2[1] + "." + m2[2]); dec.push(neg ? -v : v); return; }
      const d = w.replace(/\D/g, "");
      if (d.length >= 3 && d.length <= 5) { const v = parseFloat(d.slice(0, -2) + "." + d.slice(-2)); dec.push(neg ? -v : v); }
    });
    if (dec.length !== 5) return; // 5 sayı = L,a,b,C,h satırı
    const pre = bolum === "ref" ? "r" : "orn";
    try { $(`${pre}-L-${m[1]}`).value = dec[0]; $(`${pre}-a-${m[1]}`).value = dec[1]; $(`${pre}-b-${m[1]}`).value = dec[2]; } catch (e) {}
    if (bolum === "ref") refN++; else ornN++;
  });
  ham.textContent += `\n\nYerleştirilen: referans ${refN}, örnek ${ornN} satır. KONTROL ET — yanlış kutuyu elle düzelt, sonra hesapla.`;
};
